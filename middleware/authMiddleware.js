...
function requireRole(allowedRoles = []) {
	return (req, res, next) => {
		...
		if (!role || !allowedRoles.includes(role)) {
			...
			const isBrowserPageRequest = req.method === 'GET' && acceptHeader.includes('text/html');

			if (isBrowserPageRequest) {
				return res.redirect('/auth/login?error=Role access denied. Please login with the correct role');
			}
